...
var BuildableProto = {
	...
	build: function(startPos) {
		var state = { params: [], nextAlias: 1, nextPos: 1, text: undefined };
...
Query.prototype = {
	...
	toString: function() {
		var select = this._select.length ? 'SELECT '+this._select.join(', ') : 'SELECT *';
		...
		return `(${select}${from}${join}${where}${group}${order}${limit}${offset})`;
	},
};
...
Insert.prototype = {
	...
	toString: function() {
		...
		return `INSERT INTO ${this.rel} (${colstr}) VALUES (${valuestr})${returning}`;
	},
};
...
Update.prototype = {
	...
	toString: function() {
		...
		var updatestr = updates.join(', ');
		var where = this._where.length ? ' WHERE '+this._where.join(' AND ') : '';
		var returning = this._returning.length ? ' RETURNING '+(this._returning.join(', ')) : '';
		
		return `UPDATE ${this.rel} SET ${updatestr}${where}${returning}`;
	},
};
...
function makeChild(child, literalArgs?) {
	if (Array.isArray(child)) {
		var list = { _children: makeChildren(child) } as any;
		list.__proto__ = {
...
function metaExpr(f, literalArgs?) {
	return function() {
		...
		var o = { _children: [self].concat(args) } as any;
		o.__proto__ = {
			toString: function() {
